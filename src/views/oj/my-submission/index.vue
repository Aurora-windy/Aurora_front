...
import { listMySubmissions } from '@/api/oj/submission'
...
const total = ref(0)
const query = reactive<SubmissionQuery>({ pageNum: 1, pageSize: 10, problemId: undefined, status: undefined, language: undefined })
...
async function loadData() {
  loading.value = true
  try {
    const result = await listMySubmissions(query)
    ...
    <a-modal v-model:visible="detailVisible" title="提交详情" width="760px" :footer="false">
      <template v-if="current">
        <a-descriptions :column="2" bordered>
          ...
          <a-descriptions-item label="错误信息">{{ current.errorMessage || '-' }}</a-descriptions-item>
        </a-descriptions>
